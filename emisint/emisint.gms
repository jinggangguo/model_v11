...
$if not set ds $set ds aggregated
...
parameter dimpshare(j,r,rr);
dimpshare(j,r,rr)=0;
loop(j,
loop(r,
if(sum(rr,vxmd(j,rr,r))>1e-8,
dimpshare(j,rr,r)=vxmd(j,rr,r)/sum(rr.local,vxmd(j,rr,r));
);
);
);

parameter totimpshare;
totimpshare(j,r)=sum(rr,dimpshare(j,rr,r));
display totimpshare;
...
$ontext
parameter demis;
demis(r)=sum(i$(not sameas(i,"ele")),eintd(r,i)*vom(i,r))
+eintd(r,"ele")*(vom("ele",r)+sum(nhw,vomnhw(nhw,r)))
+sum(i,eco2(i,"c",r)+eco2(i,"g",r)+eco2(i,"i",r));

parameter impemis;
impemis(r)=sum((i,g),sum(rr,vifm(i,g,r,"cntrd")*dimpshare(i,rr,r)*einti(rr,i)));

parameter expemis;
expemis(r)=sum(i,sum(rr,vxmd(i,r,rr)*einti(r,i)));

display demis,impemis,expemis;
$offtext

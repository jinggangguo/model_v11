...
$prod:Y(g,rs)$((vom(g,rs)>1e-8) and (gov(g,rs) or inv(g,rs)))  s:1  m:0 ele:0.5 ec(ele):1
        o:P(g,rs)		q:vom(g,rs)             a:taxrev(rs)  t:rto(g,rs)
        i:PAT(i,g,rs)		q:vafm(i,g,rs)          ec:$(e(i) and (not ele(i))) ele:$ele(i) m:$(not e(i))


*       OTHER SECTORS:
$prod:Y(g,rs)$((vom(g,rs)>1e-8) and oth(g,rs))  s:0 klem:0.5  en(klem):0.5   nele(en):1 va(klem):1
        o:P(g,rs)      q:vom(g,rs)    a:taxrev(rs)  t:rto(g,rs)
...
$prod:W(c,rs)$(rshrcons(c,rs)>1e-8)  s:1
        o:PW(c,rs)			q:(vom(c,rs)+thetals*vfms("lab",c,rs))
        i:P(c,rs)			q:(vom(c,rs))
        i:PFS("lab",c,rs)		q:(thetals*vfms("lab",c,rs))


*       INCOME:

$demand:rh(c,rs)$(rshrcons(c,rs)>1e-8)
d:PW(c,rs)				q:(vom(c,rs)+thetals*vfms("lab",c,rs))
e:P("i",rs)				q:(-vom("i",rs)*rshrinc(c,rs))
e:PFS(mf,c,rs)				q:(vfms(mf,c,rs)*(1+thetals$sameas(mf,"lab")))
...
report2("Wchange",c,"CHN","pol%")$(sum(sr$province(sr),vom(c,sr)+thetals*(vfms("lab",c,sr))))
=sum(rs$province(rs),
report2("Wchange",c,rs,"pol%")*(vom(c,rs)+thetals*(vfms("lab",c,rs)))

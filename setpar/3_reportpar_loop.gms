...
* Prices:
	...
	outputdtrd	Production destined for domestic trade markets (quantity)
        ...
        nucl		Nuclear electric output (mtce)
        hydr		Hydro electric output (mtce)
	wind		Wind electric output (mtce)
	ele_nhw
	...
	en_eleinput Electricity input into energy aggregation (mtce)
        eeii             Total energy demand by production (mtce)
        eeci            Energy demand by household (mtce)
        eecii             Total energy demand by household tpye (mtce)
...
*	Fixed factor endowments for backstop vintages:
	...
	energy_prodcons  "Report parameter: energy production and consumption (mtce)"


	elec_preg	"Regional electricity generation by type (mtce)"
	prim_ener	"Primary energy use (mtce) --use efficiency of fossil electricity to convert to PE equivalent"
